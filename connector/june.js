...
const {getMessage, getNoContextMessage, addSystemMessage, addUserMessage, addAssistantMessage, 
  deleteSystemPrompt, getState} = require('@utils/gpt_user_chat_store');
...
const processMessage = async ( system_prompt_name, inputParams = {}) => {
  ...
  try {
    ...
    const { data: { choices } } =
      await gpt_chat_http(context ? getMessage(system_prompt_name) :
        getNoContextMessage(system_prompt_name), inputParams);
...
async function send_message_server(request_args){
  try {
    ...
    if(promptMappingObject?.user_prompt.includes('@user_input_message@')){
      addUserMessage(system_prompt_name,
        promptMappingObject?.user_prompt.replace('@user_input_message@', message)
      );
    }else{
      addUserMessage( system_prompt_name, message);
    }
    processMessage(system_prompt_name, inputParams);
    res.status(200).send('Message Added to queue. Processing...');
  } catch(error) {
...
function on_message_received(callback){
  const listener = (data) => {
    callback(data);
    console.debug(`Response sent for clientId: ${clientId}`);
  }
...
async function submit_feedback(feedback_argument){
  try {
    ...
    if(isSuccess){
      ...
      return 'Feedback submitted failed.';
    }
  } catch(error) {
    console.error('Error occurred while processing POST request: ', error);
